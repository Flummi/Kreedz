...
#define PLUGIN "[Kreedz] WRBot"
#define VERSION "1.3"
#define AUTHOR "Flummi / Garey - Destroman Edited"
...
enum source_data {
	...
	source_type,
	source_name[32], //name
	source_time[32], //block & type
	source_mapname[64], //mapname
	source_path[128],
	source_startframe,
...
new dl_sources[MAX_SOURCES][dl_data];

new RARArchive[128], g_szCurrentMap[32];
static g_FOUNDED_COMMUNITY;
...
new const g_szDemoFiles[][][] = {
	{ "demofilesurl", "demosfilename", "rarfilelink", "community"},
	{ "https://xtreme-jumps.eu/demos.txt", "addons/amxmodx/data/kz_downloader/demos.txt", "http://files.xtreme-jumps.eu/demos", "Xtreme-Jumps"},
	{ "https://cosy-climbing.net/demoz.txt", "addons/amxmodx/data/kz_downloader/demoz.txt", "https://cosy-climbing.net/files/demos", "Cosy-Climbing"}
};

enum _:RecordDemo {
	URLS, DEMOS, LINK, NAME
};
...
public plugin_init() {
	...
	kz_register_cmd("say /bot","kzbot_settings");
	kz_register_cmd("say /replay","kzbot_settings");
	kz_register_cmd("say /demo","kzbot_settings");
...
public kz_top_new_pro_rec(id, Float:fTime) {
	...
	fnConvertTime(fTime, sClimbTime, charsmax(sClimbTime));
	_save_run(id, sClimbTime);
}
...
public Check_Download_Demos(Community, bool:Download, bool:Filetype) {
	...
	if(!Filetype) {
		...
		new destname[128];
		format(destname, charsmax(destname), "%s_%s_%s.rar", dl_sources[src_num][dl_mapname], dl_sources[src_num][dl_name], dl_sources[src_num][dl_time]);

		new Link[256];
		format(Link, charsmax(Link), "%s/%s", g_szDemoFiles[Community][LINK], destname);

		curl_easy_setopt(curl, CURLOPT_URL, Link);

		new archivefile[128];
		format (archivefile, charsmax(archivefile), "%s/%s", archive_dir, destname);
...
public complete(CURL:curl, CURLcode:code, data[]) {
	...
	if(data[2]) {
		...
		format(destname, charsmax(destname), "%s_%s_%s", dl_sources[src_num][dl_mapname], dl_sources[src_num][dl_name], dl_sources[src_num][dl_time]);

		new filename[128];
		format(filename, charsmax(filename), "%s/%s.rar", archive_dir, destname);

		if (iResponceCode > 399 && !CheckAgain) {
			delete_file(filename);
			bFoundDemo = false;
			CheckAgain = true;
			OnDemosComplete(Community);
			return;
		}
...
public OnDemosComplete(Community) {
	new demoslist[128];
	format(demoslist, charsmax(demoslist), "%s", g_szDemoFiles[Community][DEMOS]);
	...
	while (!feof(iDemosList)) {
		...
		new parsedmap[64], DLMap[64], Extens[32], Mapa[32];
		...
		if (containi(parsedmap, g_szCurrentMap) == 0 && Time > 0.0) {
			...
			if(equali(Mapa, g_szCurrentMap)) {
				...
				if(containi(parsedmap, "[") > -1) {
					format(DLMap, charsmax(DLMap), "%s[%s", Mapa, Extens);
					if(CheckAgain)
						strtolower(DLMap);
				}
				else {
				...
				formatex(dl_sources[src_num][dl_name], charsmax(dl_sources[][dl_name]), ExplodedString[2]);
				formatex(dl_sources[src_num][dl_mapname], charsmax(dl_sources[][dl_mapname]), DLMap);

				fnConvertTime(Time, dl_sources[src_num][dl_time], charsmax(dl_sources[][dl_time]));
			}
		}
	}
	fclose(iDemosList);
	if(!already_parsed(dl_sources[src_num][dl_name], dl_sources[src_num][dl_time])) {
...
@OnComplete(id, iError) {
	if(iError != AA_NO_ERROR) {
		...
		new destname[128];
		format(destname, charsmax(destname), "%s/%s_%s_%s.dem", temp_dir, dl_sources[src_num][dl_mapname], dl_sources[src_num][dl_name], dl_sources[src_num][dl_time]);
		...
		if(is_valid_demo_file(file)) {
			if(read_demo_header(file)) {
				...
				formatex(bot_sources[sources_num][source_name], 32, "%s", dl_sources[src_num][dl_name]);
				formatex(bot_sources[sources_num][source_time], 32, "%s", dl_sources[src_num][dl_time]);
...
public forward_think(ent) {
	...
	if (equal(classname, rec_demoparser)) {
		...
		if(Finished) {
			...
			src_num--;
			if(src_num > 0) {
				Check_Download_Demos(g_FOUNDED_COMMUNITY, true, true); 
			}
			new demo_time[32];
		...
		new Float:nextthink = 0.01; //new Float:nextthink = 1//0.009;
...
public _save_run(id, fTime[32]) {
	delayed_save(id, fTime);
}
